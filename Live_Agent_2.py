...
functions = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Detect if the user is confused and needs to connect to a live agent",
            "parameters": {
                "type": "object",
                "properties": {
...
async def get_status(thread_id: str, run_id: str):
    ...
    if (run.status == "requires_action"):
        ...
        available_functions = {
            ...
            "detect_confusion": is_user_confused
        }
        ...
        for tool_call in tool_calls:
            ...
            elif function_name == "detect_confusion":
                is_confused = await function_to_call(
                    function_args.get("user_message")
                )
                if is_confused:
                    function_response = connect_to_live_agent(
                        function_args.get("user_message")
                    )
                else:
                    function_response = chatbot.get_response(
                        user_input=function_args.get("user_message")
                    )
